...
// Importar scrapers y cron job
const scrapers = require('./scrapers/coordinator');
const updateJob = require('./jobs/updateData');
...
// =======================
// Scrapers: ejecución manual por endpoint
// =======================
app.post("/scrapers/run", async (req, res) => {
  const { country } = req.body || {};
  try {
    let result;
    if (country) {
      result = await scrapers.run(country);
    } else {
      result = await scrapers.runAll();
    }
...
// =======================
app.get("/scrapers/status", async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT g.code, g.name_es, d.last_update, d.status, d.records_updated, d.error_message
       FROM data_updates d
...
app.listen(port, () => {
  ...
  // Iniciar cron job de actualización automática
  updateJob.start();
});