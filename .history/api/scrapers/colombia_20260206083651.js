 ...
 *    API gratuita api-resultadosloterias.com
 *
...
// ═══════════════════════════════════════════════════════════
// SCRAPER 2: LOTERÍAS TRADICIONALES (api-resultadosloterias.com)
// Medellín, Boyacá, Bogotá
// ═══════════════════════════════════════════════════════════

const LOTERIAS_MAP = {
  'loteria-medellin': 'medellin',
  'loteria-boyaca': 'boyaca',
  'loteria-bogota': 'bogota',
};

async function scrapeLoteriasTradicionales() {
  console.log('\n[CO] 🎟️ Consultando loterías tradicionales...');
  const saved = [];

  for (const [gameCode, slug] of Object.entries(LOTERIAS_MAP)) {
    try {
      const { data } = await axios.get(
        `https://api-resultadosloterias.com/api/lotteries/${slug}/results`,
        { headers: HEADERS, timeout: 15000 }
      );
      ...
      if (data.status === 'success' && data.data) {
        ...
        for (const result of results.slice(0, 5)) {
          const numero = String(result.number || result.resultado || '');
          const serie = String(result.serie || result.series || '');
          const fecha = result.date || result.fecha || null;

          if (!numero || numero.length < 4) continue;

          const digits = numero.split('').map(Number);
          const drawDate = fecha
            ? fecha.split('T')[0]
            : new Date().toISOString().split('T')[0];

          const numbers = {
            main: digits,
            extra: serie ? serie.split('').map(Number) : [],
          };
